...
-type valid_message(OPG) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, OPG}.

-type websocket_message(OPH) :: {valid, valid_message(OPH)} | invalid.
...
-type handler_message(OPI) :: {internal, gramps@websocket:frame()} | {user, OPI}.

-type websocket_state(OPJ) :: {websocket_state,
        bitstring(),
        OPJ,
...
-file("/home/alex/gleams/mist/src/mist/internal/websocket.gleam", 351).
...
-file("/home/alex/gleams/mist/src/mist/internal/websocket.gleam", 358).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(OQR))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(OQR))).
...
-spec apply_frames(
    ...
    fun((OQH, websocket_connection(), handler_message(OQI)) -> gleam@otp@actor:next(OQI, OQH)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(OQI), OQH),
    fun((OQH) -> nil)
) -> gleam@otp@actor:next(websocket_message(OQI), OQH).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {OPS,
        gleam@option:option(gleam@erlang@process:selector(OPT))}),
    fun((OPS) -> nil),
    fun((OPS, websocket_connection(), handler_message(OPT)) -> gleam@otp@actor:next(OPT, OPS)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(OPT))} | {error, nil}.
