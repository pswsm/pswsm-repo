...
-opaque builder(PCD, PCE) :: {builder,
        integer(),
        fun((gleam@http@request:request(PCD)) -> gleam@http@response:response(PCE)),
...
-type websocket_message(PCF) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, PCF}.
...
-spec new(
    fun((gleam@http@request:request(PDC)) -> gleam@http@response:response(PDE))
) -> builder(PDC, PDE).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(PDI, PDJ), integer()) -> builder(PDI, PDJ).
port(Builder, Port) ->
    erlang:setelement(2, Builder, Port).

-file("/home/alex/gleams/mist/src/mist.gleam", 387).
-spec read_request_body(
    builder(bitstring(), PDO),
    integer(),
    gleam@http@response:response(PDO)
) -> builder(mist@internal@http:connection(), PDO).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(PFH)
) -> {ok, websocket_message(PFH)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((PFN, mist@internal@websocket:websocket_connection(), websocket_message(PFO)) -> gleam@otp@actor:next(PFO, PFN)),
    fun((mist@internal@websocket:websocket_connection()) -> {PFN,
        gleam@option:option(gleam@erlang@process:selector(PFO))}),
    fun((PFN) -> nil)
...
-spec server_sent_events(
    ...
    fun(() -> gleam@otp@actor:init_result(PGC, PGD)),
    fun((PGD, sse_connection(), PGC) -> gleam@otp@actor:next(PGD, PGC))
) -> gleam@http@response:response(response_data()).
