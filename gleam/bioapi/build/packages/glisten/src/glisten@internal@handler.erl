...
-type message(IRT) :: {internal, internal_message()} | {user, IRT}.

-type loop_message(IRU) :: {packet, bitstring()} | {custom, IRU}.

-type loop_state(IRV, IRW) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(IRV)),
        glisten@transport:transport(),
        IRW}.

-type connection(IRX) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(IRX))}.

-type handler(IRY, IRZ) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(IRY), IRZ, connection(IRY)) -> gleam@otp@actor:next(loop_message(IRY), IRZ)),
        fun((connection(IRY)) -> {IRZ,
            gleam@option:option(gleam@erlang@process:selector(IRY))}),
        gleam@option:option(fun((IRZ) -> nil)),
        glisten@transport:transport()}.

-file("/home/alex/gleams/glisten/src/glisten/internal/handler.gleam", 73).
-spec start(handler(ISJ, any())) -> {ok,
        gleam@erlang@process:subject(message(ISJ))} |
