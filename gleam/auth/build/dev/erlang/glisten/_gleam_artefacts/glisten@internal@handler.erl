...
-type message(CSZ) :: {internal, internal_message()} | {user, CSZ}.

-type loop_message(CTA) :: {packet, bitstring()} | {custom, CTA}.

-type loop_state(CTB, CTC) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(CTB)),
        glisten@transport:transport(),
        CTC}.

-type connection(CTD) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(CTD))}.

-type handler(CTE, CTF) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(CTE), CTF, connection(CTE)) -> gleam@otp@actor:next(loop_message(CTE), CTF)),
        fun((connection(CTE)) -> {CTF,
            gleam@option:option(gleam@erlang@process:selector(CTE))}),
        gleam@option:option(fun((CTF) -> nil)),
        glisten@transport:transport()}.

-file("/home/pswsm/code/pswsm-repo/gleam/auth/build/packages/glisten/src/glisten/internal/handler.gleam", 73).
-spec start(handler(CTP, any())) -> {ok,
        gleam@erlang@process:subject(message(CTP))} |
