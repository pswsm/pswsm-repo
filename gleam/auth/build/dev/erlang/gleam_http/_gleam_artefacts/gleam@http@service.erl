...
-file("/home/pswsm/code/pswsm-repo/gleam/auth/build/packages/gleam_http/src/gleam/http/service.gleam", 17).
-spec map_response_body(
    fun((DZW) -> gleam@http@response:response(DZI)),
    fun((DZI) -> DZJ)
) -> fun((DZW) -> gleam@http@response:response(DZJ)).
map_response_body(Service, Mapper) ->
    fun(Req) -> _pipe = Req,
        _pipe@1 = Service(_pipe),
        gleam@http@response:map(_pipe@1, Mapper) end.

-file("/home/pswsm/code/pswsm-repo/gleam/auth/build/packages/gleam_http/src/gleam/http/service.gleam", 26).
-spec prepend_response_header(
    fun((EAA) -> gleam@http@response:response(EAC)),
    binary(),
    binary()
) -> fun((EAA) -> gleam@http@response:response(EAC)).
...
-file("/home/pswsm/code/pswsm-repo/gleam/auth/build/packages/gleam_http/src/gleam/http/service.gleam", 34).
-spec ensure_post(gleam@http@request:request(DZN)) -> {ok,
        gleam@http@request:request(DZN)} |
    {error, nil}.
ensure_post(Req) ->
...
-file("/home/pswsm/code/pswsm-repo/gleam/auth/build/packages/gleam_http/src/gleam/http/service.gleam", 41).
...
-file("/home/pswsm/code/pswsm-repo/gleam/auth/build/packages/gleam_http/src/gleam/http/service.gleam", 52).
-spec method_override(fun((gleam@http@request:request(EBJ)) -> EBR)) -> fun((gleam@http@request:request(EBJ)) -> EBR).
