...
-type pool(JEB, JEC) :: {pool,
        fun((glisten@internal@handler:loop_message(JEB), JEC, glisten@internal@handler:connection(JEB)) -> gleam@otp@actor:next(glisten@internal@handler:loop_message(JEB), JEC)),
        integer(),
        fun((glisten@internal@handler:connection(JEB)) -> {JEC,
            gleam@option:option(gleam@erlang@process:selector(JEB))}),
        gleam@option:option(fun((JEC) -> nil)),
        glisten@transport:transport()}.
