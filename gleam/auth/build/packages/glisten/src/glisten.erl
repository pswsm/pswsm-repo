...
-type message(JKH) :: {packet, bitstring()} | {user, JKH}.
...
-type connection(JKI) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(JKI))}.

-opaque handler(JKJ, JKK) :: {handler,
        glisten@socket@options:interface(),
        fun((connection(JKJ)) -> {JKK,
            gleam@option:option(gleam@erlang@process:selector(JKJ))}),
        fun((message(JKJ), JKK, connection(JKJ)) -> gleam@otp@actor:next(message(JKJ), JKK)),
        gleam@option:option(fun((JKK) -> nil)),
        integer(),
        boolean(),
        boolean()}.
...
-spec convert_on_init(
    fun((connection(JLY)) -> {JMA,
        gleam@option:option(gleam@erlang@process:selector(JLY))})
) -> fun((glisten@internal@handler:connection(JLY)) -> {JMA,
    gleam@option:option(gleam@erlang@process:selector(JLY))}).
...
-spec handler(
    fun((connection(JMG)) -> {JMI,
        gleam@option:option(gleam@erlang@process:selector(JMG))}),
    fun((message(JMG), JMI, connection(JMG)) -> gleam@otp@actor:next(message(JMG), JMI))
) -> handler(JMG, JMI).
handler(On_init, Loop) ->
    {handler, loopback, On_init, Loop, none, 10, false, false}.

-file("/home/alex/gleams/glisten/src/glisten.gleam", 188).
-spec map_user_selector(gleam@erlang@process:selector(message(JLN))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(JLN)).
...
-spec convert_loop(
    fun((message(JLS), JLT, connection(JLS)) -> gleam@otp@actor:next(message(JLS), JLT))
) -> fun((glisten@internal@handler:loop_message(JLS), JLT, glisten@internal@handler:connection(JLS)) -> gleam@otp@actor:next(glisten@internal@handler:loop_message(JLS), JLT)).
convert_loop(Loop) ->
    fun(Msg, Data, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec bind(handler(JNH, JNI), binary()) -> handler(JNH, JNI).
